...
async function handlePrintMemberSlip(event, { memberId, deviceName }) {
  const isDev = !app.isPackaged;
  const printWindow = new BrowserWindow({
    width: 300, // Width for a standard thermal printer
    height: 900,
  ...
  if (isDev) {
    printWindow.loadURL(`http://localhost:5173/#/print/member-slip/${memberId}`);
  } else {
    const filePath = path.join(__dirname, 'dist/index.html');
    const url = `file://${filePath}#/print/member-slip/${memberId}`;
    console.log(`[Print Window] Attempting to load URL in production: ${url}`);
    printWindow.loadURL(url);
  }
...
app.whenReady().then(async () => {
  ...
  ipcMain.handle('printMemberSlip', handlePrintMemberSlip);
})
